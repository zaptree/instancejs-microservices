...
class CoreBaseMessenger{
	dispatch(message, _request, injectables){
		...
		return this.injector.get('$schemas')
			.then(($schemas)=>{
				var schema = $schemas[message.schema];

				if(schema){
					var result = schema.validate(_request);
					if(!result.success){
						throw new Error('Validation failed I should show some helpful messages');
					}
					request = result.data;
				}
