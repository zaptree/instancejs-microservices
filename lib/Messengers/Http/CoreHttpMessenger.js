...
class CoreHttpMessenger extends include('CoreBaseMessenger'){
		...
		return this.getHostApp(config.options.port, config.options.host)
			.then((app) => {
				...
				router.use((req, res) =>{
					...
					// adding some injectables that should not be used unless you don't want multi message type compatibility
					var injectables = {
						$context: context,
						$request: req,
						$response: res,
	...
	stop(){
		throw new Error('Implement stopping the messenger');
	}
}
