...
class CoreHttpMessenger extends include('CoreMessenger') {
	constructor(CoreHttpRouter, $paths, $injector, CoreLogger, CoreBroker, $global) {
		super();
		...
		// we want to save the servers object between all services
		$global.CoreHttpMessenger = $global.CoreHttpMessenger || { servers: {} };
		this.global = $global.CoreHttpMessenger;
	}

	incomingResponse(response, options) {
		var message = super.incomingResponse(response, options);

		// cookies are not just key, value but also have options (i.e. path)
		var cookies = {};
		_.each(options.cookies, function (args) {
			cookies[args[0]] = {
				value: args[1],
				options: args[2]
			};
	...
	getPortApp(port) {
		if (!this.global.servers[port]) {
			...
			this.global.servers[port] = new Promise((resolve) => {
				var server = app.listen(port, () => {
					...
					this.global.servers[port] = srv;
					resolve(srv);
				});
			});
		}
		return this.global.servers[port];
	}
	...
	handleRequest(req, res, httpRouter) {
		...
		if (!match) {
			this.logger.error(new Error('CoreHttpMessenger [404]'), context);
			res.status(404);
		...
		this.broker.dispatch(this, match.message, msg, injectables)
			.then((response)=> {
				...
				if(response.statusCode >= 400 && [401, 403].indexOf(response.StatusCode) === -1){
					this.logger.error(new Error('CoreHttpMessenger ['+response.statusCode+']'), context);
				}
