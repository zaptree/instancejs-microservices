...
class CoreRabbitMQMessenger extends include('CoreMessenger'){
	...
	handleRequest(reqJSON, messageConfig) {
		var req;
		// fixme: move this before the handleRequest and log appropriate error stating bindings
		try {
										...
										.then((result)=>{
											if(!settings.consumerOptions.noAck){
												if(result.statusCode < 400){
													...
													console.error('RabbitMQ consumer returned status code ' + result.statusCode + ' for bindings: ' + bindings.join(','));
													channel.nack(msg);
												}
