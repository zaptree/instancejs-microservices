...
class MicroServices {
	...
	environmentRequire(fullPath, allowNotFound) {
		...
		_.each(hierarchy, function (ext) {
			...
			try {
				...
				if (error.code !== 'MODULE_NOT_FOUND' || (!ext && !allowNotFound)) {
					throw error;
				}
			}
		});
	...
	loadServices(servicePaths) {
		...
		_.map(servicePaths, (path) => {
			...
			var messages = _.merge(this.environmentRequire(Path.join(path, 'messages/messages'), true), config.messages, this.settings.messages);
	...
	startServices(services) {
		var promises = _.map(services, function (service) {
			if (!service.running) {
				console.log(service.name + ' service starting up...');
