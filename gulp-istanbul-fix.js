		...
		if(fileMap[file]){
			if(fileMap[file] === true){
				instrumenter.instrument(_code, file, function (err, code) {
					...
					// I though this code was relevant but it does not seem to be so
					//var instrumentedSrc = resultCode.toString();
					//var covStubRE = /\{.*"path".*"fnMap".*"statementMap".*"branchMap".*\}/g;
					//var covStubMatch = covStubRE.exec(instrumentedSrc);
					//if (covStubMatch !== null) {
					//  var covStub = JSON.parse(covStubMatch[0]);
					//  global[COVERAGE_VARIABLE] = global[COVERAGE_VARIABLE] || {};
					//  global[COVERAGE_VARIABLE][path.resolve(file)] = covStub;
					//}
